...
class OAuthAdapter {
    ...
    public function getRequestToken()
    {
        ...
        $res = $client->post(
                ...
                ['form_params' => ['oauth_callback' => $this->config['callbackUrl']]]
        );
    ...
    public function getAccessToken(Token\RequestToken $requestToken) {
        ...
        if($requestToken->getOauthToken() === $authToken) {
            ...
            $res = $client->post(self::ACCESS_TOKEN_URL, ['form_params' => ['user_id' => $userId]]);
    ...
    public function prepareClientForOAuthRequests(Token\AccessToken $accessToken) {
        $stack = HandlerStack::create();
        $subscriber = $this->bibsonomySubscriber->getOAuthSubscriber($accessToken);
        $stack->push($subscriber);
